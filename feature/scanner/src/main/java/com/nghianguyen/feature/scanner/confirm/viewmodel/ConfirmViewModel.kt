...
/**
 * ViewModel for the puzzle confirmation screen.
 * Responsible for validating the scanned Sudoku puzzle and creating a new game session.
 *
...
class ConfirmViewModel(
    ...
    /**
     * Initializes the state with the list of scanned digits.
     * Attempts to solve the board to verify its validity.
     *
    ...
    fun setScannedDigits(scannedDigits: List<ScannedDigitCell>) {
        ...
        sudokuSolver.solveBoard(buildSudokuGrid)
            .onSuccess { digits ->
                val givenItems = digits.filter { it.isGiven }
                    .toDigitCellItems()
                updateState {
                    copy(
                        digits = digits,
    ...
    private fun confirmSudoku() {
        ...
        launch {
            sudokuGameRepository.newGame(uiState.value.digits)
                .onSuccess {
                    Log.d("ConfirmViewModel", "new game id: $it")
                    sendEvent(ConfirmEvent.ConfirmedGame(it))
                }
                .onFailure {
                    Log.d("ConfirmViewModel", "Error: $it")
                }
        }
    }
}
