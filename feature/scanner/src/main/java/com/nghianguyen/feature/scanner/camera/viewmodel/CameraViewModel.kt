 ...
 * Orchestrates the camera lifecycle, photo capture, and the subsequent digit scanning process.
 *
...
class CameraViewModel(
    ...
    override fun onStart() {
        launch {
            cameraManager.surfaceRequest
                .collect { surfaceRequest ->
                    updateState {
                        copy(surfaceRequest = surfaceRequest)
                    }
                }
        }
    }
    ...
    private val onImageCaptured: (Bitmap) -> Unit = {
        launch {
            sudokuScanner.scanForDigits(it)
                ...
                .onFailure {
                    Log.d("CameraViewModel", "scanForDigits onfailure: $it")
                }
        }
    }
    ...
    private fun bindToCamera(lifecycleOwner: LifecycleOwner) {
        launch {
            cameraManager.bindToLifecycle(lifecycleOwner, onImageCaptured)
        }
    }
